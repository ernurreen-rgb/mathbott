...
def setup_friends_routes(app, db, limiter: Limiter):
    ...
    async def create_friend_invite(request: Request, invite_request: FriendInviteCreateRequest):
        user = await db.users.get_user_by_email(invite_request.email)
        if not user:
            user = await db.users.create_user_by_email(invite_request.email)

        active_invites = await db.friends.list_invites_by_inviter(user["id"], status="active")
        ...
        for invite in active_invites:
            ...
            if expires_at:
                ...
                if expires_at_dt and expires_at_dt > now:
                    ...
                    # Mark as expired
                    await db.friends.expire_invite(invite["token"])
            else:
        ...
        invite = await db.friends.create_invite(user["id"], expires_at)
        return {
            "token": invite["token"],
    ...
    async def list_friend_invites(
        ...
        invites = await db.friends.list_invites_by_inviter(user["id"], status=status)
        return {
            "items": invites
        }
    ...
    async def get_invite_details(
        ...
        invite = await db.friends.get_invite_by_token(token)
        if not invite:
        ...
        if status == "active" and expires_at_dt and datetime.utcnow() > expires_at_dt:
            await db.friends.expire_invite(token)
            status = "expired"

        inviter = await db.users.get_user_by_id(invite["inviter_id"])
        if not inviter:
        ...
        if email and status == "active":
            ...
            if user["id"] != inviter["id"]:
                blocked = await db.friends.is_blocked_between(user["id"], inviter["id"])
                is_friend = await db.friends.are_friends(user["id"], inviter["id"])
    ...
    async def accept_invite(token: str, accept_request: FriendInviteAcceptRequest):
        ...
        user = await db.users.get_user_by_email(accept_request.email)
        if not user:
            user = await db.users.create_user_by_email(accept_request.email)

        if user["id"] == inviter["id"]:
            raise HTTPException(status_code=400, detail="Вы не можете принять свое собственное приглашение")

        if await db.friends.is_blocked_between(user["id"], inviter["id"]):
            raise HTTPException(status_code=403, detail="Friendship not allowed")

        if await db.friends.are_friends(user["id"], inviter["id"]):
            await db.friends.mark_invite_accepted(token, user["id"])
            return {"success": True, "already_friends": True}

        await db.friends.create_friendship(user["id"], inviter["id"])
        await db.friends.mark_invite_accepted(token, user["id"])
        await db.friends.create_friend_request(inviter["id"], user["id"], status="accepted")

        return {"success": True}

    @app.post("/api/friends/invites/{token}/revoke")
    async def revoke_invite(token: str, email: str = Query(...)):
        inviter = await db.users.get_user_by_email(email)
        if not inviter:
            inviter = await db.users.create_user_by_email(email)
        ...
        await db.friends.revoke_invite(token, inviter["id"])
        return {"success": True}
    ...
    async def list_friends(email: str = Query(...)):
        ...
        if not user:
            user = await db.users.create_user_by_email(email)
        friends = await db.friends.list_friends(user["id"])
    ...
    async def remove_friend(friend_id: int, email: str = Query(...)):
        ...
        if not user:
            user = await db.users.create_user_by_email(email)
        await db.friends.delete_friendship(user["id"], friend_id)
        return {"success": True}
    ...
    async def list_friend_requests(
        ...
        if direction == "outgoing":
            items = await db.friends.list_outgoing_requests(user["id"])
        else:
            items = await db.friends.list_incoming_requests(user["id"])
        return {"items": items}

    @app.post("/api/friends/requests")
    async def create_friend_request(request: FriendRequestCreateRequest):
        sender = await db.users.get_user_by_email(request.email)
        if not sender:
            sender = await db.users.create_user_by_email(request.email)

        if sender["id"] == request.receiver_id:
            raise HTTPException(status_code=400, detail="Cannot add yourself")

        receiver = await db.users.get_user_by_id(request.receiver_id)
        if not receiver:
            raise HTTPException(status_code=404, detail="User not found")

        if await db.friends.is_blocked_between(sender["id"], receiver["id"]):
            raise HTTPException(status_code=403, detail="Friendship not allowed")

        if await db.friends.are_friends(sender["id"], receiver["id"]):
            raise HTTPException(status_code=400, detail="Already friends")

        pending = await db.friends.get_pending_request_between(sender["id"], receiver["id"])
        if pending:
            raise HTTPException(status_code=400, detail="Request already pending")

        await db.friends.create_friend_request(sender["id"], receiver["id"], status="pending")
        return {"success": True}
    ...
    async def get_friend_status(
        ...
        blocked = await db.friends.is_blocked_between(user["id"], other_id)
        is_friend = await db.friends.are_friends(user["id"], other_id)
        pending = await db.friends.get_pending_request_between(user["id"], other_id)
    ...
    async def decline_friend_request(request_id: int, email: str = Query(...)):
        ...
        request = await db.friends.get_friend_request_by_id(request_id)
        if not request:
            raise HTTPException(status_code=404, detail="Request not found")
        if request["receiver_id"] != user["id"]:
            raise HTTPException(status_code=403, detail="Not allowed")

        await db.friends.update_friend_request_status(request_id, status="declined")
        return {"success": True}
    ...
    async def accept_friend_request(request_id: int, email: str = Query(...)):
        ...
        if await db.friends.is_blocked_between(user["id"], sender_id):
            raise HTTPException(status_code=403, detail="Friendship not allowed")

        await db.friends.create_friendship(user["id"], sender_id)
        await db.friends.update_friend_request_status(request_id, status="accepted")
        return {"success": True}
    ...
    async def cancel_friend_request(request_id: int, email: str = Query(...)):
        ...
        await db.friends.update_friend_request_status(request_id, status="cancelled")
        return {"success": True}

    @app.post("/api/friends/blocks")
    async def block_user(block_request: FriendBlockRequest):
        user = await db.users.get_user_by_email(block_request.email)
        if not user:
            user = await db.users.create_user_by_email(block_request.email)

        if user["id"] == block_request.blocked_user_id:
            raise HTTPException(status_code=400, detail="Cannot block yourself")

        await db.friends.block_user(user["id"], block_request.blocked_user_id)
        return {"success": True}
    ...
    async def unblock_user(blocked_id: int, email: str = Query(...)):
        ...
        await db.friends.unblock_user(user["id"], blocked_id)
        return {"success": True}
    ...
    async def list_blocked_users(email: str = Query(...)):
        ...
        items = await db.friends.list_blocked_users(user["id"])
        return {"items": items}
